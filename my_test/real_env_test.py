...
from test_robot import Robot_control
...
class RealEnv_test:
    def __init__(self, 
            ...
            video_crf=21,
            # vis params
            enable_multi_cam_vis=True, #True
            multi_cam_vis_resolution=(640,480),#(1280,720)
            # shared memory
            shm_manager=None,
            robot_latency = 0.001
            ):
        ...
        robot = Robot_control(shm_manager = shm_manager,robot_ip=robot_ip,robot_latency = robot_latency)
    ...
    def exec_actions(self, 
        ...
        new_actions = actions[is_new]  #action is a array, is new is also a array, output is where is_new == ture
...
# recording API
    def start_episode(self, start_time=None):
        ...
        self.realsense.restart_put(start_time=start_time) #这里的start_time 就是保证和录取数据的accumulator 的 start time 一致，后面可以同采样周期采取图像帧
    ...
    def end_episode(self):
        ...
        if self.obs_accumulator is not None:
            ...
            # 因为都是相同开始时间和采样周期，所以所有数据天然就是对齐的，只要取共有的一段就可以
            #图像就无所谓了，直接录像就行，只要保证开始录的时间一样就可以，后面按采样时间来截取帧就行
